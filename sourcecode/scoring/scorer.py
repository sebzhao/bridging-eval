...
class Scorer(ABC):
    ...
    def score(
        ...
        ratings, noteStatusHistory = self._filter_input(ratingsRaw, noteStatusHistoryRaw, userEnrollmentRaw)
        ...
        noteScores = noteScores.drop(columns=["raterParticipantId_same", "raterParticipantId_interval"])
    ...
    Missing expected columns that should've been in userScores: {set(self.get_helpfulness_scores_cols()) - set(userScores.columns)}"""
        ...
        return (
            ...
            userScores[self.get_helpfulness_scores_cols()] if self.get_helpfulness_scores_cols() else None,
            noteScores[self.get_auxiliary_note_info_cols()] if self.get_auxiliary_note_info_cols() else None,
        )
