...
from . import note_status_history
...
def read_from_strings(
    ...
    """
    notes = pd.read_csv(StringIO(notesStr), sep="\t", names=c.noteTSVColumns, dtype=c.noteTSVTypeMapping)
    ratings = pd.read_csv(StringIO(ratingsStr), sep="\t", names=c.ratingTSVColumns, dtype=c.ratingTSVTypeMapping)
...
def tsv_parser(rawTSV: str, mapping: Dict[str, type], columns: List[str], header: bool) -> pd.DataFrame:
...
def user_enrollment_parser(rawTSV: str, header: bool) -> pd.DataFrame:
    ...
    try:
        ...
        df = tsv_parser(rawTSV, c.userEnrollmentExpandedTSVTypeMapping, c.userEnrollmentExpandedTSVColumns, header)
    return df
...
def read_from_tsv(
    ...
    if ratingsPath is None:
        ...
        assert len(ratings.columns.values) == len(c.ratingTSVColumns) and all(ratings.columns == c.ratingTSVColumns), (
    ...
    if userEnrollmentPath is None:
        ...
        userEnrollment = tsv_reader(userEnrollmentPath, None, None, header=headers, parser=user_enrollment_parser)
...
def _filter_misleading_notes(
    ...
    ratings[deletedButInNSHKey] = ratings[deletedNoteKey] & np.invert(pd.isna(ratings[createdAtMillisNSHKey]))

    deletedNotInNSH = (ratings[deletedNoteKey]) & pd.isna(ratings[createdAtMillisNSHKey])
    notDeletedNotMisleadingOldUI = (ratings[c.classificationKey] == c.noteSaysTweetIsNotMisleadingKey) & (
        ratings[createdAtMillisNSHKey] <= c.notMisleadingUILaunchTime
    )
    notDeletedNotMisleadingNewUI = (ratings[c.classificationKey] == c.noteSaysTweetIsNotMisleadingKey) & (
        ratings[createdAtMillisNSHKey] > c.notMisleadingUILaunchTime
    )
    ...
    ratings = ratings[ratings[notDeletedMisleadingKey] | ratings[deletedButInNSHKey] | notDeletedNotMisleadingNewUI]
...
def remove_duplicate_notes(notes: pd.DataFrame) -> pd.DataFrame:
    ...
    assert numNotes == numUniqueNotes, f"Found only {numUniqueNotes} unique noteIds out of {numNotes} notes"
...
def filter_ratings(
    ...
    notesWithMinNumRatings = n[n[0] >= minNumRatersPerNote]
    ratingsForTraining = ratingsDoubleFiltered.merge(notesWithMinNumRatings[[c.noteIdKey]], on=c.noteIdKey)
    if logging:
        print(
