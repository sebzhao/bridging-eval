...
from . import incorrect_filter, scoring_rules, tag_filter
...
def is_crnh_diamond(
    ...
    return (scoredNotes[c.numRatingsKey] >= minRatingsNeeded) & (
        ...
        <= crnhThresholdIntercept + crnhThresholdNoteFactorMultiplier * np.abs(scoredNotes[c.internalNoteFactor1Key])
    )
...
def get_ratings_before_note_status_and_public_tsv(
    ...
    ratingsWithNoteLabelInfo = ratings[
        ...
        noteStatusHistory[[c.noteIdKey, c.createdAtMillisKey, c.timestampMillisOfNoteMostRecentNonNMRLabelKey]],
        on=c.noteIdKey,
        how="left",
    ...
    if doTypeCheck:
        ...
        ratingsWithNoteLabelInfoTypes[c.createdAtMillisKey + "_note"] = np.float  # float because nullable after merge.
    ...
    ratingsWithNoteLabelInfo[c.ratingCreatedBeforePublicTSVReleasedKey] = (
        ratingsWithNoteLabelInfo[c.createdAtMillisKey] - ratingsWithNoteLabelInfo[c.createdAtMillisKey + "_note"]
    ...
    first5RatingsOldNotes = (
        ratingsWithNoteLabelInfo[
            (noteCreatedBeforeNoteStatusHistory & ratingsWithNoteLabelInfo[c.ratingCreatedBeforePublicTSVReleasedKey])
...
def get_ratings_with_scores(
    ...
    ratingsWithScores = ratingsBeforeNoteStatus[[c.raterParticipantIdKey, c.helpfulNumKey, c.noteIdKey]].merge(
        scoredNotes[
            [
                c.noteIdKey,
...
def get_valid_ratings(
    ...
    """
    ratingsWithScores = get_ratings_with_scores(ratings, noteStatusHistory, scoredNotes, logging, doTypeCheck)
    ratingsWithScores[c.ratingCountKey] = 1

    binaryRatingsOnNotesWithStatusLabels = ratingsWithScores[
        (ratingsWithScores[c.currentlyRatedHelpfulBoolKey] | ratingsWithScores[c.currentlyRatedNotHelpfulBoolKey])
        & ((ratingsWithScores[c.helpfulNumKey] == 1) | (ratingsWithScores[c.helpfulNumKey] == 0))
    ].copy()

    helpfulRatingOnCrhNote = (binaryRatingsOnNotesWithStatusLabels[c.currentlyRatedHelpfulBoolKey]) & (
        binaryRatingsOnNotesWithStatusLabels[c.helpfulNumKey] == 1
    )
    notHelpfulRatingOnCrhNote = (binaryRatingsOnNotesWithStatusLabels[c.currentlyRatedHelpfulBoolKey]) & (
        binaryRatingsOnNotesWithStatusLabels[c.helpfulNumKey] == 0
    )
    helpfulRatingOnCrnhNote = (binaryRatingsOnNotesWithStatusLabels[c.currentlyRatedNotHelpfulBoolKey]) & (
        binaryRatingsOnNotesWithStatusLabels[c.helpfulNumKey] == 1
    )
    notHelpfulRatingOnCrnhNote = (binaryRatingsOnNotesWithStatusLabels[c.currentlyRatedNotHelpfulBoolKey]) & (
        binaryRatingsOnNotesWithStatusLabels[c.helpfulNumKey] == 0
    )
...
def compute_note_stats(ratings: pd.DataFrame, noteStatusHistory: pd.DataFrame) -> pd.DataFrame:
    ...
    * During meta scoring we may invoke compute_note_stats with the full set of ratings
    ...
    last28Days = (
        1000
        * (
            datetime.fromtimestamp(c.epochMillis / 1000, tz=timezone.utc) - timedelta(days=c.emergingWriterDays)
        ).timestamp()
    )
    ratingsToUse = pd.DataFrame(ratings[[c.noteIdKey] + c.helpfulTagsTSVOrder + c.notHelpfulTagsTSVOrder])
...
def compute_scored_notes(
    ...
    # I can add in another parameter which is a specific personalized score?
    noteStats.to_csv("noteStats.tsv", sep="\t", index=False)
    import sys
    ...
    if finalRound:
        ...
        incorrectAggregates = incorrect_filter.get_incorrect_aggregates(ratings, noteParams, raterParams)
